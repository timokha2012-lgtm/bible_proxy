...
<style>
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .key-section { background: var(--surface); border: 1px solid rgba(201,169,110,0.3); border-radius: var(--radius); padding: 1rem 1.2rem; margin-bottom: 1.5rem; }
  .key-section.hidden { display: none; }
  .key-row { display: flex; gap: 10px; align-items: center; flex-wrap: wrap; margin-bottom: 6px; }
  .key-label { font-size: 12px; color: var(--gold); white-space: nowrap; }
  .key-input { flex: 1; min-width: 180px; background: var(--surface2); border: 1px solid var(--border2); border-radius: 8px; padding: 8px 12px; font-size: 13px; font-family: 'Manrope', sans-serif; color: var(--text); outline: none; }
  .key-btn { padding: 8px 16px; background: var(--gold); color: var(--bg); border: none; border-radius: 8px; font-size: 13px; font-weight: 600; font-family: 'Manrope', sans-serif; cursor: pointer; }
  .key-hint { font-size: 11px; color: var(--text3); }
  ...
  .q-loading { display: none; padding: 10px 14px; font-size: 12px; color: var(--text3); border-top: 1px solid var(--border); background: rgba(201,169,110,0.04); }
...
<div class="container">
  ...
  <div class="key-section" id="key-section">
    <div class="key-row">
      <span class="key-label">🔑 API ключ:</span>
      <input type="password" class="key-input" id="key-input" placeholder="Вставь ключ — sk-ant-..." />
      <button class="key-btn" onclick="saveKey()">Сохранить</button>
    </div>
    <div class="key-hint">Ключ сохранится в браузере — вводить повторно не нужно</div>
  </div>
...
<script>
const PROXY = 'https://bibleproxy-production.up.railway.app/proxy';
...
async function toggleQuestion(item, question) {
  ...
  try {
    const text = await callAPI([{
      role: 'user',
      content: `Ты — ведущий реабилитационной группы, дьякон-психолог. Текст: «${currentRef}». Вопрос для группы: «${question}». Дай мини-проповедь для ведущего — 4-6 предложений: что здесь главное, как направить разговор, какой духовный или психологический момент не упустить. Живым языком.`
    }], 600);
    answer.textContent = text;
    answer.classList.add('open');
  } catch(e) {
    answer.textContent = 'Не удалось загрузить подсказку: ' + e.message;
    answer.classList.add('open');
  } finally {
...
async function analyze() {
  if (!apiKey) {
    ...
    document.getElementById('error').textContent = 'Сначала введи API ключ вверху и нажми Сохранить';
  ...
  const prompt = `Разбери библейский текст «${ref}» для реабилитационной группы зависимых. Контекст: ${ctxLabel}.

Ответь ТОЛЬКО валидным JSON без markdown:
{"reference":"ссылка","verse":"цитата на русском","historical":"культурно-исторический контекст, 150 слов, живым языком","psychological":"психологический слой, механизмы, связь с зависимостью, 150 слов","theological":"богословский смысл для зависимого человека, 100 слов","application":"применение в группе, конкретно, 100 слов","questions":["вопрос 1","вопрос 2","вопрос 3","вопрос 4","вопрос 5"]}`;

  try {
    const text = await callAPI([{ role: 'user', content: prompt }], 3000);
    const match = text.match(/\{[\s\S]*\}/);
    if (!match) throw new Error('Не удалось получить ответ. Попробуй ещё раз.');
    ...
    (parsed.questions || []).forEach(q => {
      ...
      loadEl.textContent = '✍️ Готовлю подсказку ведущего...';
